...
def get_spkdic(cm_meta: str) -> Dict:
    ...
    for line in l_cm_meta:
        spk, filename, _, _, ans = line.strip().split(" ")  #strip() Remove the specified characters from the beginning and end of a str 
        # return value like "LA_0039 LA_E_2834763 - A11 spoof"
        if spk not in d_spk:
            d_spk[spk] = {}
            d_spk[spk]["bonafide"] = []
            d_spk[spk]["spoof"] = []
