...
def init_args(args):
	...
	args.log_save_path    = os.path.join(args.save_path, 'train.log')
...
def accuracy(output, target, topk=(1,)):
	...
	_, pred = output.topk(maxk, 1, True, True)  # max value crosppoding to index
	pred = pred.t()
	correct = pred.eq(target.view(1, -1).expand_as(pred))  # return bool type (which location is equation)
	res = []
	for k in topk:
